...
class AtualizacaoController extends Controller
{
    public function index()
    {
        ...
        for ($i = 0; $i < count($atualizacoes); $i++) {
            ...
            $atualizacoes[$i]['descricao'] = mb_substr($atualizacoes[$i]['descricao'], 0, 150) . '...';
    ...
    public function ver($id = -1)
    {
        ...
        if (!$id || $id == null || $id <= 0) {
            return $this->showMessage('Não encontrado', 'A atualização que você procura não pode ser encontrada', BASE . 'atualizacao/', 404);
        }
