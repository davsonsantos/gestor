...
class LoginController extends Controller
{
    ...
    public function logout()
    {
        ...
        return $this->showMessage(
            'Desconectado',
            $msg != '' ? $msg : 'Faça login para entrar novamente.',
            BASE . 'login/'
        );
    }
    ...
    public function auth()
    {
        ...
        if (!$usuario || strlen($usuario) < 6 || $usuario == '') {
            return $this->showMessage('Formulário inválido', 'O usuário informado está mal formatado', BASE . 'login/', 422);
        }

        if (!$senha || strlen($senha) < 7 || $senha == '') {
            return $this->showMessage('Formulário inválido', 'A senha informada está mal formatada', BASE . 'login/', 422);
        }

        $usuarioDados = (new LoginModel())->getUserLoginData($usuario);

        if (!isset($usuarioDados['id']) || $usuarioDados['id'] == null || $usuarioDados['id'] <= 0) {
            return $this->showMessage('Usuário inválido', 'O usuário ou senha informado é inválido', BASE . 'login/', 422);
        }
